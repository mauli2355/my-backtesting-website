...
def create_plot(data_df, buy_signals, sell_signals, stock_name, strategy_name):
    ...
    if buy_dates_in_df:
        # Buy सिग्नल थेट कँडलच्या खाली
        fig.add_trace(go.Scatter(x=buy_dates_in_df, y=data_df.loc[buy_dates_in_df]['Low'] - (data_df.loc[buy_dates_in_df]['Low'] * 0.005), 
                                 mode='markers', marker=dict(color='#2ecc71', size=12, symbol='triangle-up', 
                                                             line=dict(width=1, color='DarkSlateGrey')), 
                                 name='Buy Signal', hoverinfo='x+y+name'), row=1, col=1)
    if sell_dates_in_df:
        # Sell सिग्नल थेट कँडलच्या वर
        fig.add_trace(go.Scatter(x=sell_dates_in_df, y=data_df.loc[sell_dates_in_df]['High'] + (data_df.loc[sell_dates_in_df]['High'] * 0.005), 
                                 mode='markers', marker=dict(color='#e74c3c', size=12, symbol='triangle-down', 
                                                             line=dict(width=1, color='DarkSlateGrey')), 
                                 name='Sell Signal', hoverinfo='x+y+name'), row=1, col=1)
    ...
    fig.update_layout(
        ...
        # चार्टचा आकार मोठा करण्यासाठी (Full-width)
        autosize=True,
        height=700, # चार्टची उंची वाढवणे
        margin=dict(l=50, r=50, t=80, b=50) # मार्जिन
    )
